document.addEventListener("DOMContentLoaded", function () {
  const botaoSobel = document.getElementById("sobel");

  botaoSobel.addEventListener("click", function () {
    const dadosImagem = obterImagem("input-imagem-1");

    if (dadosImagem) {
      const dadosEscalaCinza = converterParaEscalaCinza(dadosImagem);
      const dadosProcessados = aplicarSobel(dadosEscalaCinza);
      exibirImagemNoCanvas(dadosProcessados, "canvas-output");
      console.log("Detecção de bordas Sobel aplicada");
    } else {
      alert(
  ...
  function aplicarSobel(dadosImagem) {
